import React from "react";
import { Link } from "react-router-dom";
import {useState} from 'react'

function HomePage () {
    return (
        <div className="App content-center grid h-screen place-items-center">
            <div className="h-80 w-1/3">
            <img className="w-12 h-8 m-2" src="data:image/png;base64," />

        <h3 className="text-4xl text-left font-bold"> See what's happening in the world right now </h3>

        <h5 className="text-left m-2 text-xl"> Join twitter today </h5>

        <Link to="/register">
        <button class="bg-blue-500 hover:bg-blue-700 m-1 w-full text-white font-bold py-2 px-4 rounded-full focus:outline-none focus:shadow-outline" type="button">
        Sign up
      </button>
      </Link>
        <br></br>
        <Link to="/login">
      <button class="bg-blue-500 hover:bg-blue-700 m-1 w-full text-white font-bold py-2 px-4 rounded-full focus:outline-none focus:shadow-outline" type="button">
        Log in
      </button>
      </Link>
            </div>
        </div>
    )
}

export default HomePage;